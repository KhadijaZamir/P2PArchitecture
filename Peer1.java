...
public class Peer1 {
    ...
    public static void main(String[] args) {
        if (args.length < 3) {
            System.out.println("Usage: java P2Pv2.Peer1 <localPort> <remotePort> <peerIP>");
            return;
        }
        ...
        startPeer(localPort, remotePort, peerIP, "Peer2");
    }
    ...
    public static void startPeer(int localPort, int remotePort, String peerIP, String peerName) {
        ...
        Thread serverThread = new Thread(() -> {
            try (ServerSocket serverSocket = new ServerSocket(localPort)) {
                System.out.println("Peer1 (Server) listening on port " + localPort);
                while (true) {
                    try (
